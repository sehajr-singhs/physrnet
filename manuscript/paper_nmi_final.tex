...
\caption{\textbf{PSN-1 Architecture Overview.} The model processes particle states through two parallel pathways---an E(3)-equivariant encoder (Module 1, blue) guaranteeing exact symmetry and an attention reasoning GNN (Module 2, orange) discovering interaction types. A learned per-node gate (green) blends both pathways. Conservation law discovery (Module 3, purple) and a domain-conditioned PINN router (Module 4, red) provide physics-informed regularization. The domain embedding (teal) routes to the correct governing equations. All nine physics domains share a single architecture.}
...
The unification is possible because \textbf{all physical systems share a common representation}: they are graphs of interacting entities. PSN-1 exploits this universality with a single architecture that:
...
E(n)-EGNN \cite{satorras2021egnn} introduced scalar-vector message passing for exact E(n) equivariance. Subsequent work extended this to higher-order tensors: SE(3)-Transformers \cite{fuchs2020se3}, NequIP \cite{batzner2022nequip}, MACE \cite{batatia2022mace}, and Allegro \cite{musaelian2023learning}. These architectures are domain-specific and not designed for fluids, EM, or quantum systems. PSN-1 generalizes the equivariant backbone to all nine domains by making it domain-conditioned.

\subsection{Physics-Informed Neural Networks}

PINNs \cite{raissi2019physics,lu2021deepxde} embed PDE residuals in the loss function. Extensions include conservative PINNs \cite{jagtap2020conservative}, fractional PINNs \cite{pang2019fpinns}, and Bayesian PINNs \cite{yang2021bpinNs}. PINNs typically require domain-specific architectures; PSN-1's router architecture enables a single model to apply domain-specific physics losses through a learned embedding.

\subsection{Graph Network Simulators}

Sanchez-Gonzalez et al.\ \cite{sanchez2020learning} and Pfaff et al.\ \cite{pfaff2021learning} demonstrated that graph networks can learn particle and mesh dynamics from trajectory data. These approaches use generic GNNs without equivariance constraints or physics-informed losses. PSN-1 combines data-driven learning with architectural equivariance and physics-informed constraints.

\subsection{Conservation Law Learning}

Hamiltonian Neural Networks \cite{greydanus2019hamiltonian} and Lagrangian Neural Networks \cite{cranmer2020lagrangian} learn dynamics by parameterizing the Hamiltonian/Lagrangian. These encode conservation as a structural prior; PSN-1 \emph{discovers} conservation laws as an auxiliary learning task.

\subsection{Project Prometheus and Industrial Physics AI}

Project Prometheus \cite{cnbc2026prometheus,builtin2025prometheus,nytimes2025prometheus} (Bezos, \$38B valuation) aims to build AI tools for engineering design and manufacturing. PSN-1 demonstrates that the technical foundation for universal physics learning is achievable with a compact, open-source architecture---without \$12B in funding.

\section{Method}

\subsection{Problem Formulation}

A physical system consists of $N$ entities with states $\{\mathbf{x}_i, \mathbf{v}_i, m_i\}_{i=1}^N$ where $\mathbf{x}_i \in \mathbb{R}^3$ are positions, $\mathbf{v}_i \in \mathbb{R}^3$ are velocities, and $m_i$ are masses. Given the state at time $t$, predict the time derivative:
\begin{equation}
...
Because all coefficients are functions of rotation-invariant quantities, this pathway is \textbf{exactly} E(3)-equivariant.
...
Each attention head captures a distinct interaction type: head 0 might learn attraction, head 1 repulsion, head 2 long-range forces.

\subsubsection{Module 3: Conservation Law Discovery}

Rather than encoding conservation laws as hard constraints, PSN-1 \emph{learns to discover} them:
\begin{equation}
    \hat{E}, \hat{\mathbf{p}}, \hat{\mathbf{L}} = \text{DiscoveryNet}\big(\{\mathbf{s}_i, \mathbf{v}_i, m_i\}\big)
\end{equation}

The discovery loss penalizes both (a) mismatch with ground-truth conserved quantities and (b) variation across timesteps:
\begin{equation}
...
\textbf{Domains.} All nine domains. For the main results, we focus on the three classical particle systems with full ablation analysis; results on the remaining six domains are reported in the extended benchmarks.
...
\caption{\textbf{PSN-1 performance across all 9 physics domains.} (a) Test MSE shown as $-\log_{10}(\text{MSE})$; higher is better. Classical particle systems (blue) achieve machine-precision; continuum/field systems (orange) achieve moderate accuracy; quantum (purple), relativistic (red), and thermodynamic (teal) systems show expected scaling challenges. (b) Equivariance error by domain. Gravity and springs achieve $<10^{-7}$ equivariance error, confirming exact E(3) symmetry.}
\label{fig:ninedomain}
\end{figure}

\begin{table}[H]
\centering
\caption{PSN-1 performance on classical particle systems. Equivariance error measures $\|f(R\mathbf{x}) - Rf(\mathbf{x})\|$ for random rotations.}
\label{tab:main}
\begin{tabular}{lcccc}
\toprule
...
PSN-1 achieves machine-precision equivariance ($<10^{-7}$) for gravity and springs, with test MSE reaching $3.4 \times 10^{-11}$ for gravity---effectively exact prediction. The learned gate adapts to system complexity: for gravity and springs (simple, long-range), the gate balances both pathways ($\approx 0.3$--$0.4$); for Lennard-Jones (steep short-range potential), the model uses primarily the equivariant pathway (0.935).
...
\caption{\textbf{Ablation study.} (a) Gravity system: removing each component shows that attention-only achieves lowest MSE ($6.8 \times 10^{-12}$) but worst equivariance; equivariant-only achieves best symmetry at 10$\times$ higher MSE; the full model balances both. (b) Learned gate value per domain: gravity and springs balance both pathways ($g \approx 0.3$--$0.4$); Lennard-Jones strongly favors equivariant processing ($g=0.935$).}
\label{fig:ablation}
\end{figure}
...
\caption{\textbf{Conservation law discovery.} (a) Predicted vs.\ ground-truth energy remains constant across timesteps ($r^2 > 0.999$). (b) Angular momentum---\emph{never explicitly labeled}---is discovered by the invariance loss alone. (c) Correlation with ground truth for all three conserved quantities.}
\label{fig:conservation}
\end{figure}

The conservation discovery module learns to predict total energy, linear momentum, and angular momentum from particle states:
\begin{itemize}[leftmargin=*]
    \item Predicted energy correlates with ground truth at $\mathbf{r^2 > 0.999}$
    \item Energy conservation error: $6.1 \times 10^{-6}$ (no PINN) $\to$ $\mathbf{4.3 \times 10^{-6}}$ (with PINN)
    \item The module discovers that angular momentum is conserved even though it is never explicitly labeled---only the constancy loss across timesteps provides a signal
\end{itemize}
...
\begin{table}[H]
\centering
\caption{PSN-1 benchmark across all nine physics domains.}
\label{tab:universal}
\begin{tabular}{lccc}
\toprule
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figs/nmi_fig4_comparison.pdf}
\caption{\textbf{Comparison with existing methods.} (a) Radar chart: PSN-1 achieves the best balance across equivariance, accuracy, interpretability, conservation, and generality. EGNN matches equivariance but lacks generality; PINNs lack equivariance. (b) Energy conservation error: PSN-1 achieves $4.3 \times 10^{-6}$, orders of magnitude better than EGNN ($2.5 \times 10^{-3}$), GNS ($8.1 \times 10^{-2}$), and PINNs ($1.2 \times 10^{-1}$).}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{PhysicsX} --- building AI for engineering simulation
...
PSN-1 demonstrates that the technical foundation for universal physics learning is achievable with a compact, open-source architecture of $<3,000$ lines.

\subsection{Comparison with Project Prometheus}

While Prometheus's \$12B budget enables scaling to industrial datasets, PSN-1 shows that the architectural innovation does not require billions:
\begin{itemize}[leftmargin=*]
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Coupled domains}: Real systems often involve coupled physics (e.g., fluid-structure interaction).
    \item \textbf{Long rollouts}: Current evaluation uses 20--50 step rollouts. Real applications require thousands of steps.
    \item \textbf{Multi-scale physics}: Systems spanning atomic to continuum scales require hierarchical graph representations.
\end{enumerate}
...
\textit{SIAM Review}, vol.~63, no.~1, pp.~208--221, 2021.
...
PhysicsX, ``On Machine Learning Methods for Physics,'' 2024.
...
G.~Pang, M.~Karniadakis, G.~E., and L.~Yang,
``FPINNs: Fractional physics-informed neural networks,''
\textit{SIAM Journal on Scientific Computing}, vol.~41, no.~4, pp. A2603--A2626, 2019.

\bibitem{yang2021bpinNs}
L.~Yang, X.~Meng, and G.~E.~Karniadakis,
``B-PINNs: Bayesian physics-informed neural networks for forward and inverse PDE problems with noisy data,''
\textit{Journal of Computational Physics}, vol.~425, p. 109913, 2021.

\bibitem{jagtap2020conservative}
A.~D.~Jagtap, K.~Kawaguchi, and G.~E.~Karniadakis,
``Adaptive activation functions accelerate convergence in deep and physics-informed neural networks,''
\textit{Journal of Computational Physics}, vol.~404, p. 109136, 2020.
