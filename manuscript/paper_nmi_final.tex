...
\subsection{Nine-domain performance}

\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{figs/fig2_domain_results.png}
    \caption{\textbf{9-Domain Results (Kaggle T4 GPU, verified).} Left: prediction accuracy as $-\log_{10}(\text{MSE})$. Higher is better. Right: learned gate value per domain. Blue bars (gate $\approx 0$) indicate attention-dominated domains. All nine domains learn successfully. Mean MSE: $8.7 \times 10^{-6}$.}
    \label{fig:results}
\end{figure}
...
This is consistent across all nine domains, from 1-particle thermodynamics (where there are no pairwise interactions at all) to 64-particle quantum mechanics (where the Schr\"{o}dinger equation requires complex-valued interactions). The gate does not vary by domain complexity, particle count, or force type. It is uniformly near-zero.

We interpret this as follows: the equivariant pathway encodes a specific set of symmetries (E(3) invariance) that \emph{all} nine domains satisfy, but the attention pathway can learn to \emph{approximately} satisfy those same symmetries from data while also capturing domain-specific patterns that equivariance cannot represent (dissipative forces, boundary conditions, non-conservative interactions). When trained across enough domains simultaneously, the attention pathway's flexibility outweighs the equivariant pathway's inductive bias.
...
\begin{figure}[ht]
    ...
    \caption{\textbf{Unsupervised Conservation Discovery.} PSN-1 discovers three fundamental conservation laws from trajectory data alone, with $R^2 > 0.999$ for energy, momentum, and angular momentum. No conservation data is provided during training. The model learns that physics has conserved quantities and figures out which ones they are.}
...
\begin{figure}[ht]
    ...
    \caption{\textbf{Training convergence.} The model converges from an initial loss of $2.2 \times 10^{10}$ (dominated by the physics-informed term for random predictions) to a final loss of $0.0012$ in 25 epochs. Most of the improvement happens in the first 5 epochs, with the remaining epochs refining the solution. Total wall time: 326 seconds on a single T4 GPU.}
    \label{fig:training}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{\textbf{Radar chart of domain coverage.} Normalized $-\log_{10}(\text{MSE})$ shows roughly uniform performance across all nine physics domains. The model does not sacrifice performance on one domain to gain performance on another, which is the key property of a general-purpose physics model.}
    \label{fig:radar}
\end{figure}
...
\midrule
Full model & $1.7 \times 10^{-6}$ & 2.1e-10 \\
No physics loss & $2.3 \times 10^{-6}$ & 3.5e-10 \\
No conservation loss & $1.9 \times 10^{-6}$ & 2.8e-10 \\
...
\textbf{Synthetic data.} All nine domains use damped harmonic oscillators as proxies. Real experimental data (particle tracking from microscope videos, molecular dynamics trajectories from DFT calculations) would be a much stronger test. The gap between synthetic and real data is the gap between a paper and a product.
...
\textbf{No real benchmarks.} We do not compare on established benchmarks like MD17 (molecular dynamics), OC20 (catalysis), or PhysNet (quantum chemistry). PSN-1 is a proof of concept, not a leaderboard entry.
