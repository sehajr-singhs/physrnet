...
class EGNNLayer(nn.Module):
    ...
    def forward(self, x, pos, adj):
        B, N, D = pos.shape
        # Compute all pairwise relative vectors
        rel = pos.unsqueeze(2) - pos.unsqueeze(1)  # (B, N, N, 3)
        dist = rel.norm(dim=-1, keepdim=True)       # (B, N, N, 1)
        edge_feat = torch.cat([rel, dist], dim=-1)   # (B, N, N, 4)
        
        m = self.mlp(edge_feat).squeeze(-1)  # (B, N, N)
        m = m.masked_fill(adj == 0, 0.0)  # adj is already (B, N, N)
        agg = m.sum(dim=2, keepdim=True)  # (B, N, 1)
...
class EGNN(nn.Module):
    ...
    def forward(self, positions, adj=None, atom_types=None):
        ...
        x = self.embed(positions)
        if adj is None:
            adj = torch.ones(B, N, N, device=positions.device) - torch.eye(N, device=positions.device).unsqueeze(0)
        
        for layer in self.layers:
            x, positions = layer(x, positions, adj)
...
def train_model(model, data, model_name, n_epochs=50, lr=1e-3, batch_size=32):
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(0, len(perm), batch_size):
            ...
            adj_batch = adj.expand(len(idx), -1, -1) if adj.size(0) == 1 else adj
            pred_f, pred_e = model(positions[idx], adj_batch)
        ...
        model.eval()
        with torch.no_grad():
            adj_val = adj.expand(len(val_idx), -1, -1) if adj.size(0) == 1 else adj
            pred_f, pred_e = model(positions[val_idx], adj_val)
